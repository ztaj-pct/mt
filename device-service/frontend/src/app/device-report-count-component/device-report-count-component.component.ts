...
@Component({
  selector: 'app-device-report-count-component',
  templateUrl: './device-report-count-component.component.html',
  styleUrls: ['./device-report-count-component.component.css']
})
export class DeviceReportCountComponentComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      {
        headerName: 'IMEI', field: 'deviceId',
        sortingOrder: ['asc', 'desc', null], unSortIcon: true,
        filter: 'agTextColumnFilter', floatingFilter: true,
        filterParams: containsFilterParams,
        width: 200,
        cellRenderer: function (params) {
          if (params.data !== undefined && params.data.deviceId != null) {
            return '<div class="pointer" style="color:#4ba9d7;"><a>' + params.data.deviceId + '</a></div>';
          }
        }
      },
      {
        headerName: 'Ms1 Record Count', field: 'ms1recordcount', unSortIcon: true, filter: 'agTextColumnFilter', width: 180, floatingFilter: true, filterParams: containsFilterParams,
        cellRenderer: function (params) {
  ...
  onGridReady(params) {
    ...
    const datasource = {
      getRows: (params: IGetRowsParams) => {
        ...
        if (this.isFirst) {
          this.loading = true;
          this.gatewayListService.gatLatestDeviceReportCountWithPage(this.currentPage, this.column, this.order, this.pageLimit,
